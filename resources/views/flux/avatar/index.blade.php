...
$classes = Flux::classes()
    ->add(match($size) {
        '2xl' => '[:where(&)]:size-20 [:where(&)]:text-lg',
    ...
    ->add($circle ? '[--avatar-radius:calc(infinity*1px)]' : match($size) {
        '2xl' => '[--avatar-radius:var(--radius-2xl)]',
    ...
    ->add($hasTextContent ? '[:where(&)]:bg-zinc-200 dark:[:where(&)]:bg-zinc-600 [:where(&)]:text-zinc-800 dark:[:where(&)]:text-white' : '')
    ->add(match($color) {
